...
for config in $(echo "${configurations}" | jq -c '.tokens[]'); do
  ...
  if ! jq -e --arg symbol "token_$symbol" '.canisters | has($symbol)' dfx.json &>/dev/null; then
    ...
    cat dfx.json | jq --arg symbol "token_$symbol" '.canisters += {($symbol): {"type": "motoko", "main": "src/token.mo"}}' > dfx.tmp
    mv dfx.tmp dfx.json

    # Execute dfx commands to create and install the new canister
    dfx canister create token_${symbol}
    dfx build token_${symbol}
    dfx canister install token_${symbol} --argument="(principal \"$owner\",\"$logo\", \"$name\", \"$symbol\", $decimals)"
...
curl -X POST -H 'Content-Type: application/json' -d "{\"canisterInfo\": \"$(echo -e "$canister_info" | tail -n 1 | sed 's/"/\\"/g')\"}" http://localhost:5004/api/scriptInfo
