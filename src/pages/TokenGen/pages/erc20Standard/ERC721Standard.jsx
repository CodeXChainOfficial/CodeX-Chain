...
const deepaikey ='c039b3f1-ca9e-4084-b084-e53ebe3f7941'
...
const handleCallAllReadFunctions = async () => {



  
  // Filter only read functions
  const readFunctions = abiArray.filter((func) => func.stateMutability === 'view');
console.log(readFunctions)
console.log("function and function input", functionInputs )
  try {
    // Iterate through each read function and call it
    for (const func of readFunctions) {
      // Call the function and get the result
      const result = await callContractFunction(func, func.inputs);

      // Update the functionOutputs array with the result
      setFunctionOutputs((prevOutputs) => [
        ...prevOutputs,
        { function: func, value: result },
      ]);
    }
  } catch (error) {
    console.error('Error calling read functions:', error);
  }
};
      ...
      .map((func, index) => (
        <div key={index} >
         <Button key={index} onClick={() => handleSelectFunction(func)}>
          ...
          {functionOutputs.find((output) => output.function === func) && (
            <div>
             {formatOutput(functionOutputs.find((output) => output.function === func).value)}
            </div>
          )}
        </div>
      ))}
    {/* Button to call all read functions at once */}
    <Button onClick={handleCallAllReadFunctions}>Call All Read Functions</Button>
  </div>
)}
